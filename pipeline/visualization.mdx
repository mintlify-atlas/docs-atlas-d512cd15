...
On failure, the `errors` array contains the last five error-matching lines from each affected log file (filtered to exclude false positives such as "0 errors"), enabling rapid post-mortem diagnosis without manually `grep`-ing through the `logs/` directory.
