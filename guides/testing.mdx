...
<CardGroup cols={2}>
  ...
  <Card title="ENCODE blacklist BED" icon="ban">
    A two-region blacklist (`data/reference/ENCODE_blacklist.bed`) covering known repeat-dense intervals.
  </Card>
...
<CardGroup cols={2}>
  <Card title="lint" icon="check">
    Runs `validate_config.py`, `snakemake --lint`, and `pytest rules/scripts/test_validate_config.py` to catch config errors and rule syntax issues before any computation starts.
  </Card>
