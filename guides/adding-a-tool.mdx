...
```yaml
# rules/envs/misc/template_tool.yaml  (exact source content)
name: template_env
...
rule template_tool:
    ...
    # [TEMPLATE] The actual bash commands to run the tool.
    shell:
        """
...
<Warning>
The CI pipeline runs `validate_config.py` as its first step. A config that fails validation will block the entire workflow from starting.
</Warning>
...
#         chrom.sizes, and ENCODE blacklist BED (no internet required)
